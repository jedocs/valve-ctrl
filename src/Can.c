...
char CANOpen(unsigned char CONFIG1, unsigned char CONFIG2, unsigned char CONFIG3) {
    ...
    CIOCON = 0x20; //Drive TX pin Vdd when recessive, disable capture !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!4
...
//    IPR3 = 0xFF;  //priority, set in main
    ...
    TXB0DLC = 0x08; //0 data bytes to send
    TXB0CON = 0x00; //Initiate transmission
    
    TXB0REQ = 1; //addig küldi folyamatosan amig meg nem érkezik vagy txbreq=0!!!!
...
char CANPutMessage(void) {
    ...
    if (TXWPtr != TXRPtr) //If there are any more data to send
    {
        ...
        RXB0RTRRO = 1; //a kontrollregiszterek ablakolása miatt ez a TXBnCON.TXREQ-nak felel meg
        return 0;
    } else
        return 1;
}
